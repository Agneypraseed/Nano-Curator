      ...
      <section id="wardrobe-history" className="space-y-5">
        <div className="flex items-end justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Wardrobe history</h2>
              <p className="text-sm text-slate-500">Your saved pieces and looks you selected from the generator.</p>
            </div>
          </div>
        ...
        {wardrobeEntries.length === 0 ? (
          <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">Add a current dress or favorite a generated look to start your wardrobe.</div>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
            {wardrobeEntries.map((entry) => (
              <article key={entry.id} className="group overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm">
                <div className="aspect-[3/4] overflow-hidden bg-slate-100">
                ...
                <div className="space-y-1 p-3">
                  <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-indigo-600">{entry.kind}</span>
                  <h3 className="truncate text-sm font-medium text-slate-800">{entry.label}</h3>
                </div>
              </article>
            ))}
          </div>
